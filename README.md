...
| Groq free tier over OpenAI | No cost, very fast inference — but dependent on a third-party free tier with a 6,000 tokens/minute limit. When several questions are processed back-to-back, cumulative input + output tokens can hit this cap mid-generation: the response is cut off, JSON parsing fails, and the question falls back to "Not found in references." Setting `max_tokens=2048` raises the ceiling per call but does not bypass the per-minute rate limit. Production would use a paid, SLA-backed model API with much higher limits. |
